                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form id="banner-form" method="post" class="p-4" enctype="multipart/form-data">
                                <div class="row">
                                    <div class="col-4 text-left offset-8">
                                        <label for="status">Status</label>
                                        <select name="bnr_status" id="bnr-status" class="form-control">
                                    ...
                                    <div class="col-6">
                                        ...
                                        <input type="hidden" id="bnr-id" name="bnr_id">
                                    ...
                                    <div class="col-4 mt-3">
                                        ...
                                        <div class="mt-2 custom-file">
                                            <input type="hidden" id="image-name" name="imageName" value="">
...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
            type: "GET",
            url: './mvc/action/banner/bannerGetAction.php?id=' + id,
            processData: false,
            contentType: false,
            success: function(data) {
                if (JSON.parse(data).result) {
                    ...
                    if(!Object.keys(getData).length) {
                        modelMessageCall('Redirecting', 'Data not available redirect to banner page .....');
                        setTimeout(function(){
                            location.href = './banner.php';
                        }, 3000);
                    } else {
                        $('#bnr-status').val(getData.bnr_status);
                        $('#bnr-id').val(getData.bnr_id);
                        $('#title').val(getData.title);
                        $('#desc').val(getData.description);
                        $('#image-name').val(JSON.stringify([getData.image_lg, getData.image_sm]));
                        $('#image-pre').attr('src', './../' + getData.image_sm);
                        $('#url').val(getData.url_redirect);
                    }
                } else {
        ...
        $("#banner-form").submit(function(e) { 
            ...
            if ($('#title').val() == '' || $('#desc').val() == '' || $('#url').val() == '') {
                ...
                $.ajax({
                    type: "POST",
                    url: './mvc/action/banner/bannerEditAction.php',
                    data: form,
                    processData: false,
                    contentType: false,
                    success: function(data) {
