                            ...
                            <form id="blog-form" method="post" class="p-4" enctype="multipart/form-data">
                                <div class="row">
                                    <div class="col-4 text-left offset-8">
                                        ...
                                        <select name="blg_status" id="blg-status" class="form-control">
...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
            type: "GET",
            url: './mvc/action/blog/blogGetAction.php?id=' + id,
            processData: false,
            contentType: false,
            success: function(data) {
                if (JSON.parse(data).result) {
                    ...
                    if(!Object.keys(getData).length) {
                        modelMessageCall('Redirecting', 'Data not available redirect to blog page .....');
                        setTimeout(function(){
                        ...
                        var imageView = getData.image_sm ? './../' + getData.image_sm: './../assets/images/temp/no-image.jpg'
                        $('#image-pre').attr('src', imageView);
                    }
                } else {
        ...
        $("#blog-form").submit(function(e) { 
            ...
            if ($('#title').val() == '' || $('#desc').val() == '' || $('#url').val() == '') {
                ...
                $.ajax({
                    type: "POST",
                    url: './mvc/action/blog/blogEditAction.php',
                    data: form,
                    processData: false,
                    contentType: false,
                    success: function(data) {
